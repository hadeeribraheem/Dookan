<header class="header d-flex align-items-center fixed-top">
    <div class="d-flex align-items-center justify-content-between me-auto">
        <a href="#" class="logo d-flex align-items-center">
            ...
            <span class="d-none d-lg-block">{{ __('keywords.dookan_seller') }}</span> <!-- Translated "Dookan Seller" -->
        </a>
        ...
        <ul class="d-flex align-items-center me-4">
            <li class="nav-item">
                <a href="?lang={{ app()->getLocale() === 'ar' ? 'en' : 'ar' }}">
                    <button type="button" class="lang-btn">
                        {{ app()->getLocale() === 'ar' ? __('keywords.english') : __('keywords.arabic') }} <!-- Translated language switch -->
                    </button>
                </a>
            </li>
            ...
            <li class="nav-item">
                <a href="{{ route('admin.profile') }}" data-toggle="dropdown" class="nav-link nav-link-lg nav-link-user">
                    ...
                    @else
                        <img src="{{ asset('images/default.png') }}" alt="image" style="width: 40px;height: 40px; object-fit: cover;" class="rounded-circle mr-1">
                    @endif

                    <div class="d-none d-md-block d-lg-inline-block">
                        <span class="text-white">{{ __('keywords.hi_user') }} {{ Auth::user()->username }} </span> <!-- Translated "Hi" -->
                    </div>
                </a>
            </li>
        </ul>
